...
uses
  ...
  IdTCPClient,
  IdHTTP,
  System.Net.HttpClient, FMX.Dialogs;
...
type
  ...
  public
    ReturnObterDados: String;
    constructor Create(ABaseURL: string = '');
    function ObtemDados(APIResource: string): Boolean; overload;
    function ObtemDados(AMemTable: TFDMemTable; APIResource: string)
      : Boolean; overload;
    function ObtemDados(APIResource: string; Carregar: Boolean)
      : String; overload;
...
begin
  ...
  FRESTResponseDataSetAdapter := TRESTResponseDataSetAdapter.Create
    (FRESTRequest);
...
begin
  try
    ...
    lUrl := FBaseURL + 'Versao/Elegance/' + APIResource + '.json';
...
function TComunicacao.ObtemDados(APIResource: string;
  Carregar: Boolean): String;
begin
  try
    FRESTClient.BaseURL := FBaseURL;
    FRESTRequest.Method := rmGET;
    FRESTRequest.resource := 'Versao/Elegance/' + APIResource + '.json';
...
begin
  try
    ...
    FMemTable.Active := true;
    result := true;
    ReturnObterDados := FResponseJson;
  except
    result := false;
  end;
